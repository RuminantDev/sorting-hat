...
export function getFlagVal(uid: string, flagName: string, flagConfig: any) {
  ...
  if (flagConfig.perc !== undefined) {
    ...
    const specialId: number = murmurhash.v3(`${uid}-${flagName}`);
...
function getFlagPercVal(hashedUid: number, perc: number) {
  const x = getBucket(hashedUid);
  console.log("X: ", x, hashedUid);
  return x < perc;
}

function getBucket(hashedUid: number) {
  const A = 1;
  const P = 50;
